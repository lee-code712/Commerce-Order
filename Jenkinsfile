def IMG = "192.168.100.12/commerce-yr/commerce-yr-order-img:order-"
def KC = "/usr/local/bin/kubectl --kubeconfig=/home/jenkins/acloud-client.conf"

pipeline {

  environment {
    registry = "lee-code712/Commerce-Order"
    dockerImage = ""
  }
  ...
  stages {
    ...
    stage('Checkout Source') {
      steps {
        ...
        git 'https://github.com/lee-code712/Commerce-Order.git'
    ...
    stage('Deploy App') {
      steps {
        script {
          ...
          sh "${KC} apply -f order_deployment_v2.yaml"
          sh "${KC} set image deployment/commerce-yr-order-v2 commerce-yr-order-v2=${IMG}${BUILD_NUMBER} -n commerce-yr"
          echo "Deploy App END"
        }
      }
    }
